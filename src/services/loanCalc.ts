...
export function calculateLoanInterest(details: LoanDetails): number {
  ...
  if (loanType === 1) {
    ...
    throw new Error(
      "Invalid loan type. Use 1 for flat loan, 2 for diminishing loan."
    );
  }
}
...
export function getDaysBetweenMonthRange(
  startStr: string,
  endStr: string
): number {
...
export function CalculateInitialInterest(data: CalaulateInterest): number {
  ...
  try {
    ...
    if (data.interestCal === 2) {
      ...
      let temp1 =
        (Number(data.principal) * (Number(data.annualInterest) * 12)) /
        100 /
        12;
